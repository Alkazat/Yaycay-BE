...
`v0.3` use-our-AI surfaces (streaming planning chat + structured ingestion), and
`v0.5` Stripe Checkout + the entitlement webhook.
...
| Endpoints | Public: `POST /demo/generate-day`, `POST /signup/capture`, **`POST /webhooks/stripe`** (signature-verified). Customer (RLS-scoped): `GET /trips` (`TripSummary[]`), `POST /trips`, `GET /trips/:id` (`TripContent`), `GET/PATCH /trips/:id/content`, `POST /auth/2fa/verify`, **`POST /trips/:id/plan/chat`** (SSE, tier=ours), **`POST /trips/:id/ingest`** (paid), **`POST /checkout/session`**. Admin: the full `/admin/*` surface. See `docs/CONTRACT-STATUS.md` for the consumer-facing index |
...
and returns problem+json. `checkout` creates a Stripe Checkout Session for a
