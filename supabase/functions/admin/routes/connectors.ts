...
const GRANT_SELECT =
  'connection_id, user_id, client_id, scope, status, revoked_at, last_used_at, created_at, oauth_clients(client_name)';
const GRANT_SELECT_INNER =
  'connection_id, user_id, client_id, scope, status, revoked_at, last_used_at, created_at, oauth_clients!inner(client_name)';
...
function toAdminConnector(r: GrantRow, email: string | undefined) {
  return {
    // The connection id is the stable connector id (survives token refresh).
    id: r.connection_id,
    userId: r.user_id,
    ownerEmail: email ?? '',
    ...
    // Prefer the explicit status; fall back to revoked_at for older rows.
    status: r.status ?? (r.revoked_at ? 'revoked' : 'active'),
...
export async function revokeConnector(
  ...
  const { data: existing, error: selErr } = await db
    .from('oauth_grants')
    .select('connection_id, status, revoked_at')
    .eq('connection_id', id)
    .maybeSingle();
  if (selErr) throw badRequest(selErr.message);
  if (!existing) throw notFound('Connector grant not found.');

  // Effective immediately: mark revoked and purge the issued-token material so
  // the store (which looks grants up by token hash and rejects non-active
  // grants) can never match a previously-issued token again.
  ...
  await writeAudit(ctx, {
    ...
    before: {
      status:
        ...
        ((existing as { revoked_at: string | null }).revoked_at ? 'revoked' : 'active'),
    },
